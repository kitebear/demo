...
function FullPage(options) {
    ...
    var pageContain = document.getElementById(options.id),//获取ID的容器
        page = pageContain.children,//获取子节点元素
        pagelen = page.length,//统计页面的总数
        iPage = pagelen,
        sTime = options.slideTime || 800,//描述事件设定
    ...
    if (!page || pagelen === 1) return;//如果没有页面或者长度为1的话 结束方法
    /**
     * 获取mode并保存在数组
     */
    if (options.mode) {
    ...
    removePage = function (className) {
        ...
        for (var i = 0; i < pagelen; i++) {
            if (page[ i ].className.indexOf(className) !== -1) {
                index = i;
            }
        }
        if (index === null || index === indexNow) {
            return;
        }
        iPage--;
        pagelen--;
